...
from models.metrics import evaluate, evaluate_mse
...
def run(input, target, info, config, path):
    ...
    for train_date, valid_date, test_date in tqdm(zip(train_dates, valid_dates, test_dates), desc='Expanding window training...'):
        ...
        models = list()
        train_loader, valid_loader, test_loader, test_index, autoencoder_model = create_dataloaders(
            input, target, info, train_date, valid_date, test_date, batch_size = config['batch_size'], embedding_method='autoencoder'
            )
        autoencoder_model_path = os.path.join(path, train_date + '_autoencoder_model0.pt')
        if(autoencoder_model is not None):
            torch.save(autoencoder_model.state_dict(), autoencoder_model_path)

        for i in tqdm(range(ensemble), desc='Ensemble model training...'):        
            
            # Train model and get forecast results with test scores
            set_random_seed(i+1)  
            model = train(config, train_loader, valid_loader, device, True)
        ...
        score_mse = evaluate_mse(c, r, c_hat, r_hat, info)
        score_table[test_date] = scores
        mse_scores[test_date] = score_mse
        del scores
        ...
        test_scores = evaluate(actual_concept, actual_target, forecast_concept, forecast_target, info)
        print(test_scores["Return"])
    ...
    score_mse = evaluate_mse(actual_concept, actual_target, forecast_concept, forecast_target, info)
    score_table['Whole periods'] = scores
    mse_scores['Whole periods'] = score_mse
    del scores
    del score_mse
    ...
    return score_table, mse_scores, output
...
def main():
    ...
    parser.add_argument("-w", "--weight",  dest="weight",  action="store", type=float,
                        help="Weight lambda for concept bottleneck loss (0 for standard neural network)")
    args = parser.parse_args()

    # Define save path
    file_name = str(args.horizon) + 'month_' + str(args.weight)
    ...
    # Load data
    input = pd.read_csv(f'data/input_{args.horizon}month.csv')
    target = pd.read_csv(f'data/target_{args.horizon}month.csv')
    ...
    print(info[info['Cat.Data'] == 'Analyst']['LongDescription'].values)
    # Run CB-APM

    print('\n------------------------------------------------------------------------------------')
    print(f'Training lambda={str(args.weight)}')
            
    score, mse, output = run(input, target, info, config, path)

    # Save training results
    score.to_csv('results/' + file_name + '.csv')
    mse.to_csv('results/' + file_name + '_mse.csv')
