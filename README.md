...
│
├── analysis/              # Model evaluation & post-hoc analysis
│   ├── util.py, stats_utils.py # Shared helpers (loading, composites, HAC/bootstrap tests)
│   ├── model_eval/             # Core model evaluation (converted from analysis/*.ipynb)
│   │   ├── r_squared_analysis.py   # R² analysis across models
│   │   ├── mse_analysis.py         # MSE analysis (in-sample & out-of-sample)
│   │   ├── portfolio_analysis.py   # Portfolio performance analysis
│   │   ├── coefficient_analysis.py # Consensus coefficient analysis
│   │   ├── consensus_analysis.py   # Consensus variable analysis
│   │   ├── hjd.py                  # Hansen-Jagannathan Distance analysis
│   │   └── GRS_analysis.py         # Gibbons-Ross-Shanken test analysis
│   ├── revision/                # Management Science revision analyses (post-hoc,
│   │   │                        # reuse final_results/ & final_checkpoints/, no retraining)
│   │   ├── mic_decomposition.py, divergence_mechanism.py, lambda_selection.py,
│   │   │   decision_value.py, robust_portfolio.py, interpretability.py, subpanel.py,
│   │   │   compression_ladder.py, stickiness_concentration.py, stickiness_robustness.py,
│   │   │   spread_robustness.py, validation_lambda_selection.py, leakage_naming.py
│   ├── experiments/             # NN-training scripts, written but not run (see DEVIATIONS.md)
│   └── outputs/                 # Generated tables/figures/summaries per experiment
│
...
The `analysis/` directory is organized by purpose. Each script exposes a `run(...)` function
callable from a `__main__` block (run as a module, e.g. `python -m analysis.model_eval.hjd`), and
writes its tables/figures/summary to `analysis/outputs/<script_name>/`. Shared helpers
(`analysis/util.py`, `analysis/stats_utils.py`) stay at the top level since both subpackages below
import from them.

### `analysis/model_eval/` -- core model evaluation

| Script | Description |
|--------|-------------|
| `r_squared_analysis.py` | Compare out-of-sample R² across models and time periods |
| `mse_analysis.py` | Analyze in-sample and out-of-sample Mean Squared Error |
| `portfolio_analysis.py` | Evaluate portfolio performance based on model predictions |
| `coefficient_analysis.py` | Analyze the coefficients of consensus variable approximations |
| `consensus_analysis.py` | Deep dive into consensus variable predictions |
| `hjd.py` | Hansen-Jagannathan Distance analysis |
| `GRS_analysis.py` | Gibbons-Ross-Shanken test for factor model evaluation |

These were converted from the original `analysis/*.ipynb` notebooks to keep the analysis pipeline
scriptable and reviewable. Notebooks were removed once their replacement script was verified to
reproduce the notebook's cached results. `coefficient.ipynb` and `consensus.ipynb` are still kept
alongside their scripts: `data/input_12month.csv`/`target_12month.csv` were regenerated by an
unrelated later fix and no longer match the values these two notebooks were last run against, so
`coefficient_analysis.py`/`consensus_analysis.py` currently reproduce the *logic* but not the
*numbers* of their notebooks -- see `analysis/outputs/DEVIATIONS.md` for the full root-cause
analysis.

### `analysis/revision/` -- Management Science revision analyses

The modules here implement the additional experiments (E5–E15) for the journal revision, built
entirely on the existing `final_results/` / `final_checkpoints/` artifacts (no new model
training). See `analysis/outputs/DEVIATIONS.md` for every documented deviation from the original
experiment spec, and `analysis/outputs/HEADLINE.md` / `analysis/outputs/E15/` for the headline
stickiness-concentration result. `analysis/experiments/` holds the NN-training scripts for E1/E2/E3
and E14's AE-compressor variants -- written but intentionally not run (see DEVIATIONS.md).
